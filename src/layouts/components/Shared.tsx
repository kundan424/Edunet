              ...
              <div className="relative" ref={accountMenuRef}>
                ...
                {isAccountMenuOpen && (
                  <div className="absolute right-0 mt-4 w-56 bg-white border-2 border-midnight-ink rounded-lg shadow-lg z-50 flex flex-col py-2">
                    ...
                    {user?.role === 'STUDENT' && <Link to="/learn" className="px-4 py-2 text-sm font-bold hover:bg-cream-paper transition-colors">My Learning</Link>}
                    {user?.role === 'INSTRUCTOR' && (
                      <>
                        <Link to="/instructor" className="px-4 py-2 text-sm font-bold hover:bg-cream-paper transition-colors">Instructor Dashboard</Link>
                        <Link to="/instructor/courses" className="px-4 py-2 text-sm font-bold hover:bg-cream-paper transition-colors">My Courses</Link>
                        <Link to="/instructor/profile" className="px-4 py-2 text-sm font-bold hover:bg-cream-paper transition-colors">Profile</Link>
                      </>
                    )}
                    {user?.role === 'ADMIN' && (
                      <>
                        <Link to="/admin" className="px-4 py-2 text-sm font-bold hover:bg-cream-paper transition-colors">Admin Dashboard</Link>
                        <Link to="/admin/courses" className="px-4 py-2 text-sm font-bold hover:bg-cream-paper transition-colors">Course Moderation</Link>
                      </>
                    )}
              ...
              <div className="mt-auto pt-8 border-t border-midnight-ink/10 flex flex-col gap-6">
                {!isAuthenticated ? (
                  ...
                  <div className="flex flex-col gap-6">
                    <div className="flex items-center gap-4 p-4 border border-midnight-ink/20 rounded-xl bg-white">
                    ...
                    <button onClick={() => { logout(); setIsMobileMenuOpen(false); }} className="w-full font-usual text-lg font-bold text-ember-red text-center py-3 border border-ember-red rounded-lg hover:bg-ember-red hover:text-white transition-colors">
                      Logout
                    </button>
                  </div>
                )}
  ...
  return (
    <footer className="border-t border-midnight-ink/10 bg-white text-midnight-ink py-12 md:py-20 mt-12 md:mt-24">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 md:px-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-12 md:gap-8">
        ...
        {/* Platform Column */}
        <div className="flex flex-col gap-4">
          <h3 className="font-degular-display text-lg font-bold uppercase tracking-widest text-signal-blue">Platform</h3>
          <Link to="/courses" className="font-usual text-sm hover:text-signal-blue transition-colors w-fit">Browse Courses</Link>
        </div>

        {/* Roles Column */}
        <div className="flex flex-col gap-4">
          <h3 className="font-degular-display text-lg font-bold uppercase tracking-widest text-signal-blue">For You</h3>
        ...
        <div className="flex flex-col gap-4">
          ...
          <Link to="/about" className="font-usual text-sm hover:text-signal-blue transition-colors w-fit">About Us</Link>
          <Link to="/contact" className="font-usual text-sm hover:text-signal-blue transition-colors w-fit">Contact</Link>
          <Link to="/terms" className="font-usual text-sm hover:text-signal-blue transition-colors w-fit">Terms</Link>
